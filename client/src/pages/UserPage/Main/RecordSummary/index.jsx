...
import LineChart from "../../../MyPage/Main/RecordSummary/LineChart";

function RecordSummary({ recordList }) {
  ...
  useEffect(() => {
    recordList.map((item, index) => {
      ...
      dateTemp[index] = `${date.getFullYear()}/${(
        "00" +
        (date.getMonth() + 1)
      ...
      if (
        licenseToValue(highRecordTemp[item.season]) <
        licenseToValue(item.license)
      )
        highRecordTemp[item.season] = item.license;

      if (recordList.length === index + 1) {
